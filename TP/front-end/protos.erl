...
-type 'MsgCS'() ::
      #{%% type                 => iolist()         % = 1
        ...
        %% repL                 => 'Reply_Login'()  % = 4
        %% orderRequest         => 'OrderRequest'() % = 5
       }.
-export_type(['Reply_Login'/0, 'Request_Login'/0, 'OrderRequest'/0, 'Client'/0, 'MsgCS'/0]).

-spec encode_msg('Reply_Login'() | 'Request_Login'() | 'OrderRequest'() | 'Client'() | 'MsgCS'(),'Reply_Login' | 'Request_Login' | 'OrderRequest' | 'Client' | 'MsgCS') -> binary().
encode_msg(Msg, MsgName) ->
    encode_msg(Msg, MsgName, []).


-spec encode_msg('Reply_Login'() | 'Request_Login'() | 'OrderRequest'() | 'Client'() | 'MsgCS'(),'Reply_Login' | 'Request_Login' | 'OrderRequest' | 'Client' | 'MsgCS', list()) -> binary().
...
e_msg_MsgCS(#{} = M, Bin, TrUserData) ->
    ...
    case M of
	  ...
	  begin
	    ...
	    e_mfield_MsgCS_orderRequest(TrF5, <<B4/binary, 42>>,
					TrUserData)
	  end;
      _ -> B4
    end.
...
merge_msg_MsgCS(PMsg, NMsg, TrUserData) ->
    ...
    case {PMsg, NMsg} of
	  ...
	  S5#{orderRequest => NForderRequest};
      {#{orderRequest := PForderRequest}, _} ->
	  S5#{orderRequest => PForderRequest};
      {_, _} -> S5
    end.
...
v_msg_MsgCS(#{} = M, Path, TrUserData) ->
    ...
    case M of
      #{orderRequest := F5} ->
	  v_msg_OrderRequest(F5, [orderRequest | Path],
			     TrUserData);
      _ -> ok
    end,
    lists:foreach(fun (type) -> ok;
...
get_msg_defs() ->
    [{{msg, 'Reply_Login'},
	 ...
	 opts => []},
       #{name => orderRequest, fnum => 5, rnum => 6,
	 type => {msg, 'OrderRequest'}, occurrence => optional,
	 opts => []}]}].


get_msg_names() ->
    ['Reply_Login', 'Request_Login', 'OrderRequest',
     'Client', 'MsgCS'].
